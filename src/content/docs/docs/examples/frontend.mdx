...
No Para-specific config needed. The transpiler emits standard `import "@lyku/para-signals"` statements that Vite resolves through node_modules:
...
```json
{
  ...
  "dependencies": { "@lyku/para-signals": "*", "@lyku/para-parallel": "*" }
}
```
