---
title: Edge example
description: A Cloudflare Worker in Para with per-request signal scope, a derived Cache-Control header, and a Durable Object rate limiter using a `when` block for once-per-window alerting.
---
