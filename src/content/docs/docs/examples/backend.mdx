...
The `parabun build --target node` invocation emits standard `@para/*` imports that Node resolves through `node_modules` like any other npm package — no bundler plugin needed.
