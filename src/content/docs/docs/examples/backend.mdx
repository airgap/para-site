...
The `parabun build --target node` invocation handles `para:*` resolution by way of an esbuild-style plugin if you have one configured, or you can replace the imports in source with the explicit `@para/<module>` path. Both work.
