...
description: A WebSocket server in Para with per-connection signals, an idle-timeout via a `when` block, and a server-wide reactive SSE stats endpoint. Runs on Node 18+.
---
...
wss.on("connection", (ws: WebSocket) => {
  ...
  signal now      = Date.now() every 1_000;   // tick so `idle` recomputes
  signal idle     = (now - lastSeen) > 60_000;
  signal closed   = false;
...
```json
{
  ...
  "dependencies": {
    "ws": "^8",
    "@lyku/para-signals": "*", "@lyku/para-parallel": "*"
  },
...
```json
{
  ...
  "type": "module",
  "scripts": {
    "build": "parabun build src/server.pjs --target node --outfile dist/server.js",
    "start": "node dist/server.js"
  },
  "dependencies": {
    "ws": "^8",
...
The `parabun build --target node` invocation emits standard `@lyku/para-*` imports that Node resolves through `node_modules` like any other npm package — no bundler plugin needed.
