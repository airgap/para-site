...
// `whenever` (initial-truthy + edge) catches a boot-already-empty tank.
// `when` (strict edge) skips the boot-false case so a healthy startup
// doesn't fake a "back above empty" alert.
whenever tankEmpty { console.error("⚠️  tank EMPTY — pausing watering"); }
...
- **`whenever tankEmpty { ... }`** + **`when not tankEmpty { ... }`** — `whenever` fires once on registration if the predicate is initially truthy AND on each false→true rise; `when` is strict-edge (no initial-truthy fire). Picking the right one matters for ops alerts: if you boot with the tank already empty, `whenever` notifies immediately; `when not tankEmpty` only fires on actual refills (no fake "back above empty" alert on a healthy boot). The runtime handles the edge tracking — the alternative is hand-rolling `wasEmpty` plus `if (empty && !wasEmpty)`.
