...
// Paired form: `whenever` catches boot-already-empty + each new empty
// event; bare `when not { }` is the inverse-edge arm (always strict-
// edge, so a healthy boot doesn't fake a recovery alert).
whenever tankEmpty { console.error("⚠️  tank EMPTY — pausing watering"); }
when not           { console.error("✓  tank back above empty"); }
