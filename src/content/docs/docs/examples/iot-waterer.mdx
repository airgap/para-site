...
// empty event; bare `when not { }` is the inverse-edge arm (always
...
- **`when tankEmpty start { ... }`** + **bare `when not { ... }`** — the trailing `start` modifier on the first arm fires once at registration if the predicate is initially truthy AND on each false→true rise; the bare-paired arm is always strict-edge (no initial-truthy fire). Picking the right one matters for ops alerts: if you boot with the tank already empty, `when tankEmpty start` notifies immediately; the bare paired `when not { }` only fires on actual refills (no fake "back above empty" alert on a healthy boot). The runtime handles the edge tracking — the alternative is hand-rolling `wasEmpty` plus `if (empty && !wasEmpty)`.
