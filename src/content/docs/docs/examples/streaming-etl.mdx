...
description: 10M-element pipeline with the |> operator + @lyku/para-simd primitives. Each |> step is one SIMD pass over the typed array. Native AVX2 / NEON in Parabun, WebAssembly SIMD elsewhere; ~5-6× faster than .map().reduce().
---
...
- [`@lyku/para-simd`](/docs/simd/) — the full kernel surface (sum / dot / matVec / topK / scalar+vector arithmetic)
- [`@lyku/para-pipeline`](/docs/pipeline/) — `|>` runtime helpers + affine-chain `compile()`
