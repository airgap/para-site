...
`Line` is `AsyncDisposable`. The reactive `line.value` Signal updates on `read()` and on edge events — pair with [`@lyku/para-signals`](/docs/signals/) for `effect { ... }` / `~>` composition.
...
- [`@lyku/para-signals`](/docs/signals/) — reactive composition over `line.value`.
