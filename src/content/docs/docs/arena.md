---
title: "@lyku/para-arena"
description: A pool of SharedArrayBuffer typed arrays. Per-chunk work doesn't allocate a fresh buffer.
---

```ts
import arena, { Pool, scope } from "@lyku/para-arena";
```

A small allocator: a pool of `SharedArrayBuffer`-backed typed arrays drawn from a pre-warmed pool, returned at the end of an `arena { }` block (or programmatically via `release()`). Used internally by [`@lyku/para-parallel`](/docs/parallel/) and [`@lyku/para-pipeline`](/docs/pipeline/) so per-chunk work doesn't allocate a fresh buffer every time.
...
```ts
import { scope } from "@lyku/para-arena";
...
`@lyku/para-arena` matters when you have:
