...
Pick `when` for "user pressed button" / "page loaded" / anything where faking the event at startup would be wrong, and `whenever` for safety, health, and connectivity alerts where missing the boot-bad case is worse than firing one extra time.
