...
```ts
import { signal, derived, effect, batch, untrack, Signal } from "@lyku/para-signals";
```

`@lyku/para-signals` is a small reactive primitive. `signal(v)` is a cell, `derived(fn)` is a read-only signal computed from others, and `effect(fn)` runs side effects when something it read changes. Reads inside an effect register a dependency; writes invalidate downstream and a microtask flush re-runs only the effects whose observed values actually changed.
...
<Tabs syncKey="lang">
  ...
  <TabItem label="ParaBun">
    ```parabun
    derived double = count * 2;   // explicit form — keyword telegraphs read-only
    signal  doubleAuto = count * 2; // also a derived (auto-promoted from RHS)
    double;            // 4 (after count = 2)
    ```
  </TabItem>
</Tabs>
...
```ts
import sigs from "@lyku/para-signals";
...
## `promiseSignal(thunk)` — promise → `{ data, error, pending }` cell

The producer behind the `.pui` `async signal x = EXPR` form. The thunk fires
**synchronously** at construction (the request starts immediately) and
receives an `AbortSignal`; `dispose()` aborts it *and* drops a late settle —
no stale state after unmount. Fire-once-per-mount by design: it does not
re-run when a dependency changes (that's `querySignal`, below).

```ts
import { promiseSignal } from "@lyku/para-signals";

const user = promiseSignal(s => fetch(`/api/user/${id}`, { signal: s }).then(r => r.json()));
user.peek();   // { data, error, pending } — pending: true until the settle
```

## `querySignal(thunk, schema, opts?)` — the tracked, gated sibling

The producer behind `derived NAME :: SCHEMA = EXPR`
([spec §10.7](/spec/07-sources-async-and-native/)): a client-pull async cell
whose settle crosses `schema.parse` — `Ok` stores the **parsed** value, `Err`
lands in `error` as a state, never a throw. One instance is one *run*: the
`.pui` bridge constructs it inside a tracked scope, so reading a signal in
the thunk makes a change re-key (dispose + fresh construct = latest-wins,
no run-id machinery). `opts.prev` seeds stale-while-revalidate — a refetch
shows the old `data` with `pending: true` instead of a flash to undefined.

```ts
import { querySignal } from "@lyku/para-signals";

const found = querySignal(s => api.search(q, { signal: s }), Post, { prev: last });
```

```parabun
// the .pui sugar — refetches when `id` changes, response gated by User
...
- **Background work**: an `effect` can dispatch a [`@lyku/para-parallel`](/docs/parallel/) `pmap` and write the result back into a signal — the next read picks it up.
...
- No ownership / scope — effects live forever unless `dispose()`d. Wrap with [`@lyku/para-arena`](/docs/arena/)-style scoping in long-lived loops.
